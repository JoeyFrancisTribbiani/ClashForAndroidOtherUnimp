...
fun Context.startClashService(clock :Long): Intent? {
    ...
    if (startTun) {
        ...
        if (vpnRequest != null)
            return vpnRequest
        var myintent =TunService::class.intent
        myintent.putExtra("myclock",clock)
        startForegroundServiceCompat(myintent)
